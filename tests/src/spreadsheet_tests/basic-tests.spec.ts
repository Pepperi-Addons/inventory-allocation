...
describe("Basic tests from spreadsheet", () => {
    ...
    //     it(`Initial submit - order a few items with ${ordersQty} orders - both should success`, async () => {   
    //       var successOrders = 0;
    //       var failOrders = 0;
    //       var userId = 0;
    ...
    //       expect(warehouse.Inventory['1']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['2']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['3']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['4']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['5']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['6']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['7']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['8']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(warehouse.Inventory['9']).to.be.equal(10000 - (successOrders * 100) + (failOrders * 0));
    //       expect(successOrders + failOrders).to.be.equal(ordersQty);
    //     });
    ...
    //   describe("18* case - Allocate the order and don't cancel it, allocation shouldn't deallocate", async () => {
    //     const warehouseID = uuid();
    //     const user = uuid();
    //     var ordersQty = 20;
    //     var orders: string[] = [];
    ...
    //     it('Wait 7 minutes', () => sleep(420 * 1000))

    //     it('Check the the warehouse and allocations - after 7 minutes', async () => {
    //       const warehouse = await apiClient.getWarehouse(warehouseID);
    //       const allocation = await apiClient.getOrderAllocations();
