    ...
    describe("User-Specific tests from spreadsheet with multiple users", () => {
      describe("31 case - basic - with sufficient qty", async () => {
        ...
        it('Initial submit - order a few items using UserAlloc and Inventory qty - should success', async () => {        
          ...
          expect(warehouse.Inventory['1']).to.be.equal(10000  - (successOrders * 2000 ? 10000 : successOrders * 2000));
          expect(warehouse.Inventory['2']).to.be.equal(10000 - (successOrders * 2000 ? 10000 : successOrders * 2000));
          expect(warehouse.Inventory['3']).to.be.equal(10000 - (successOrders * 2000 ? 10000 : successOrders * 2000));
          expect(warehouse.UserAllocations['1']).to.be.undefined;
